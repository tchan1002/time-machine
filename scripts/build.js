...
<html lang="en">
  ...
  <body>
    ...
    <footer class="site-footer">Generated by md-journal-site</footer>
    ${extraScripts}
  </body>
  </html>`;
}
  ...
  const sections = days.map(([md, arr]) => {
    ...
    const columns = byYear.map(e => `
      <div class="compare-card">
        ...
        <div class="compare-card-content">${e.html}</div>
      </div>
    `).join('\n');
    return `
      <details data-month-day="${md}">
        <summary><h2>${md}</h2></summary>
        <div class="compare-grid" style="grid-template-columns: repeat(${byYear.length}, 1fr)">\n${columns}\n</div>
      </details>`;
  }).join('\n');
